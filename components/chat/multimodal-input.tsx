...
function PureMultimodalInput({
  ...
  const submitForm = useCallback(() => {
    window.history.pushState(
      {},
      "",
      `${process.env.NEXT_PUBLIC_BASE_PATH ?? ""}/assistant/${chatId}`
    );
      ...
      try {
        ...
        if (response.ok) {
          ...
          return {
            ...
            contentType,
            kind: isImage ? ("image" as const) : ("session-document" as const),
          };
        }
          ...
          >
            {attachments.map((attachment) => (
              <PreviewAttachment
                ...
                key={attachment.url}
                onRemove={() => {
                  if (attachment.kind === "session-document" && attachment.id) {
                    fetch(
